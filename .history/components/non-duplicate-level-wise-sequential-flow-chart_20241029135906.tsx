...
export function FlowChart() {
  ...
  const createNodesAndEdges = async (
    ...
    // 确保获取到新创建的节点
    const [newNode, _] = await createNodeAndEdge(text, parentId, x, y);
    if (!newNode) return; // 添加防护检查

    if (text.length > 1 && !createdNodesRef.current.has(`${text}-decomposed`)) {
      createdNodesRef.current.add(`${text}-decomposed`);
      const parts = await decompose(text);
      // 使用 newNode.id 而不是 node.id
      const childNodes = await createLevelNodes(parts, newNode.id, x, y + 150);
  ...
  const handleGenerate = useCallback(async () => {
    ...
    await createNodesAndEdges(input, null, 0, 0);
    setIsLoading(false);
