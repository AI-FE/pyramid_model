...
export function FlowChart() {
  ...
  const createNodesAndEdges = async (
    ...
    if (isRoot) {
      // 根节点：创建新节点
      [currentNode] = await createNodeAndEdge(text, parentId, x, y);
    } else {
      // 非根节点：使用已存在的节点
      currentNode = nodes.find((node) => node.data.label === text);
      if (!currentNode) {
        console.error(`Expected to find existing node for text: ${text}`);
        return;
      }
    }
  ...
  const handleGenerate = useCallback(async () => {
    ...
    setIsLoading(false);
  }, [input, setNodes, setEdges, createNodesAndEdges]);
