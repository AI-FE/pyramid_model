...
export function FlowChart() {
  ...
  const createLevelNodes = async (
    ...
    const NODE_SPACING = 50; // 节点之间的额外间距
    const totalWidth =
      texts.length * NODE_WIDTH + (texts.length - 1) * NODE_SPACING;
    const startX = x - totalWidth / 2;

    const levelNodes: Node[] = [];
    for (let i = 0; i < texts.length; i++) {
      const nodeX = startX + i * (NODE_WIDTH + NODE_SPACING);
      const [node, _] = await createNodeAndEdge(texts[i], parentId, nodeX, y);
  ...
  const createNodesAndEdges = async (
    ...
    const parts = isRoot ? [text] : await decompose(text);
    const childNodes = await createLevelNodes(parts, parentId, x, y + 150);
    // 并行处理所有子节点
    await Promise.all(
