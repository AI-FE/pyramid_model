...
export function FlowChart() {
  ...
  const depthNodesRef = useRef<Map<number, Node[]>>(new Map());
  const depthPositionsRef = useRef<Map<number, DepthPosition>>(new Map());

  // 修改 createLevelNodes 函数
  const createLevelNodes = async (
    ...
    // 检查同一深度是否已有节点位置信息
    const positionInfo = depthPositionsRef.current.get(depth);
    let startX = x - totalWidth / 2;

    if (positionInfo) {
      const rightmostX = positionInfo.end;
    ...
    if (positionInfo) {
      depthPositionsRef.current.set(depth, {
        start: Math.min(positionInfo.start, startX),
        end: Math.max(positionInfo.end, endX),
      });
    } else {
      depthPositionsRef.current.set(depth, { start: startX, end: endX });
    }

    const levelNodes: Node[] = [];
    const existingNodes = depthNodesRef.current.get(depth) || [];
    ...
    // 更新该深度的节点列表
    depthNodesRef.current.set(depth, [...existingNodes, ...levelNodes]);
  ...
  const resetDepthNodes = () => {
    ...
    depthPositionsRef.current.clear();
  };
